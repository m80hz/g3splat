...
class LossNormal(Loss[LossNormalCfg, LossNormalCfgWrapper]):
    def forward(
        ...
        if self.cfg.lambda_context_views_normal == 0.0:
            ...
            gaussian_rotations = rearrange(gaussians.rotations, "b (v h w) d -> (b v) h w d", v=V, h=H, w=W)     # shape (B, H, W, 4)
            ...
            gs_surfel_normals = gaussian_rot_matrices[..., :, 2]  # shape: (B, H, W, 3)


            eps = 1e-6  # small constant for numerical stability
            norm_surf_ptc = torch.norm(surf_normals_ptc, dim=-1, keepdim=True)  # (B, H, W, 1)
            norm_gs_surfels = torch.norm(gs_surfel_normals, dim=-1, keepdim=True)  # (B, H, W, 1)
            ...
            loss_mean = loss_per_pixel.mean()
            context_view_loss = self.cfg.lambda_context_views_normal * loss_mean

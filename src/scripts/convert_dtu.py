''' Build upon: https://github.com/donydchen/mvsplat/blob/main/src/scripts/convert_dtu.py
                https://github.com/dcharatan/real_estate_10k_tools
...
def load_pfm(path, downSample=1.0):
    depth_h = np.array(read_pfm(path)[0], dtype=np.float32)  # (1200, 1600)
    depth_h = cv2.resize(depth_h, None, fx=0.5, fy=0.5,
                            interpolation=cv2.INTER_NEAREST)  # (600, 800)
    depth_h = depth_h[44:556, 80:720]  # (512, 640)
    depth_h = cv2.resize(depth_h, None, fx=downSample,
                            fy=downSample, interpolation=cv2.INTER_NEAREST)
    return torch.tensor(depth_h, dtype=torch.float32)
...
def load_depths(example_path: Path) -> dict[int, UInt8[Tensor, "..."]]:
    depths_dict = {}
    for cur_id in range(0, 49):
        cur_depth_name = f"depth_map_{cur_id:04d}.pfm"
        depthmap = load_pfm(example_path / cur_depth_name)
        depths_dict[cur_id] = depthmap
...
if __name__ == "__main__":
    ...
    for stage in ("test",):
        ...
        for key in keys:
            ...
            example["depths"] = [
                depths[timestamp.item()] for timestamp in example["timestamps"]
            ]
            assert len(images) == len(example["timestamps"])
            assert len(depths) == len(example["timestamps"])
